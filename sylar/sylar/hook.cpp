...
retry:
    ...
    if(n == -1 && errno == EAGAIN) {
        ...
        if(rt) {
            ...
            // 添加事件成功，从此处让出协程
            //SYLAR_LOG_DEBUG(g_logger) << "before do_io<" << hook_fun_name << ">";
            sylar::Fiber::YieldToHold();
            //SYLAR_LOG_DEBUG(g_logger) << "after do_io<" << hook_fun_name << ">";
            // 1.读到事件   2.定时器超时 两种情况切换回来
            if(timer) {
                timer->cancel();
            }
            if(tinfo->cancelled) {
...
unsigned int sleep (unsigned int secondes) {
    if(!sylar::t_hook_enable) {
        return sleep_f(secondes);
    }
    // 更改sleep实现，增加定时器任务，并切换当前协程
    sylar::Fiber::ptr fiber = sylar::Fiber::GetThis();
    sylar::IOManager* iom = sylar::IOManager::GetThis();
    //iom->addTimer(secondes * 1000, std::bind(&sylar::IOManager::scheduler<Fiber::ptr>, iom, fiber));
    iom->addTimer(secondes * 1000, std::bind((void(sylar::Scheduler::*)
...
int usleep (useconds_t usec) {
    ...
    sylar::Fiber::ptr fiber = sylar::Fiber::GetThis();
    sylar::IOManager* iom = sylar::IOManager::GetThis();
    //iom->addTimer(usec / 1000, std::bind(&sylar::IOManager::scheduler, iom, fiber));
