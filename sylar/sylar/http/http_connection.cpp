...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    do {
        ...
        //SYLAR_LOG_INFO(g_logger) << "data:" << data;
        if(len <= 0) {
            close();
            SYLAR_LOG_INFO(g_logger) << "len <= 0";
            return nullptr;
        }
        len += offset;
        data[len] = '\0';
        size_t nparser = parser->execute(data, len, false);
        //SYLAR_LOG_INFO(g_logger) << nparser;
        if(parser->hasError()) {
            close();
            SYLAR_LOG_INFO(g_logger) << "parser->hasError()";
            return nullptr;
        }
        offset = len - nparser;
        if(offset == (int)buff_size) {
            close();
            SYLAR_LOG_INFO(g_logger) << "offset == (int)buff_size";
            return nullptr;
        }
    ...
    if(client_parser.chunked) {
        ...
        do{
            ...
            //SYLAR_LOG_INFO(g_logger) << "len=" << len;
...
int HttpConnection::sendRequest(HttpRequest::ptr rsp) {
    ...
    //SYLAR_LOG_INFO(g_logger) << "sendRequest=" << data;
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    if(!ptr) {
        IPAddress::ptr addr = Address::LookAnyIPAddress(m_host);
        if(!addr) {
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if((ptr->m_createTime + pool->m_maxAliveTime) >= sylar::GetCurrentMS() 
                || !ptr->isConnected()
